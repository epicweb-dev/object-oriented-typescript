...
await testStep('User constructor should set name, email, and default role', async () => {
	...
	expect(user.email, '🚨 User.email should be "alice@example.com" - check your constructor parameter assignment').toBe('alice@example.com')
...
await testStep('User constructor should accept custom role', async () => {
	...
	expect(admin.email, '🚨 User.email should be "bob@example.com" - check your constructor parameter assignment').toBe('bob@example.com')
