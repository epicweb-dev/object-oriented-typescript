# Constructors

👨‍💼 TypeScript has a shorthand for constructor parameters that automatically
creates and assigns fields, but this workshop runtime does **not** support that
syntax. We'll use the long form here.

```ts
// Long form
class User {
	name: string
	constructor(name: string) {
		this.name = name
	}
}

// Shorthand with parameter properties (not supported in this runtime)
class User {
	constructor(public name: string) {}
}
```

## Private Fields

JavaScript has native private fields using the `#` prefix:

```ts
class BankAccount {
	#balance: number = 0 // Truly private - not accessible outside the class

	deposit(amount: number) {
		this.#balance += amount
	}
}

const account = new BankAccount()
account.#balance // ❌ Error! Private field not accessible
```

🐨 Open <InlineFile file="index.ts" /> and:

1. Create classes with explicit fields and constructor assignments
2. Use default parameter values
3. Use `#` prefix for fields that should be private

💰 Parameter property shorthand (reference only):

```ts
constructor(public name: string, public email: string) {}
```

💰 Private field syntax:

```ts
class Counter {
	#count: number = 0

	increment() {
		this.#count++
	}
}
```

📜 [MDN - Private class features](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/Private_class_fields)

🐨 Once you've completed the exercise, run `node index.ts` in the playground to test your work!
