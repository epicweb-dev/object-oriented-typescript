...
await testStep('MediaPlayer should accept MediaFile instances', async () => {
	...
	).toBe('Playing file.mp3')
})

await testStep(
	'MediaPlayer should accept AudioFile instances (polymorphism)',
	async () => {
		...
		).toBe('Playing audio: song.mp3')
	},
)

await testStep(
	'MediaPlayer should accept VideoFile instances (polymorphism)',
	async () => {
		...
		).toBe('Playing video: movie.mp4')
	},
)

await testStep(
	'AudioFile and VideoFile should be substitutable for MediaFile',
	async () => {
		const audio = new AudioFile('song.mp3')
		const video = new VideoFile('movie.mp4')
		const player = new MediaPlayer()
