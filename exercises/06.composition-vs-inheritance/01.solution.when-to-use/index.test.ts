...
await testStep(
	'EmailService should accept any Logger (composition)',
	async () => {
		const fileLogger = new FileLogger()
...
await testStep(
	'EmailService should use logger in sendEmail method',
	async () => {
		const fileLogger = new FileLogger()
...
await testStep(
	'EmailService should work with different logger types',
	async () => {
		const fileLogger = new FileLogger()
		const consoleLogger = new ConsoleLogger()
